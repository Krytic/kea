...
class histogram:
        ...
        def Fill(self, x, w=1):
            if not isinstance(x, type(0.0)):
                ...
                for i in range(0, len(x)):
                    if x[i] >= self._xup:
                         self._values[self._nr_bins-1] += w[i]
                    elif x[i] <= self._xlow:
                        self._values[0] += w[i]
                    else:
                        bin_nr = np.where(self.lower_edges <= x[i])[0][-1]
                        self._values[bin_nr] += w[i]
                # code repeat. Make a func?

            else:
                if x >= self._xup:
                    self._values[self._nr_bins-1] += w
                elif x <= self._xlow:
                    self._values[0] += w
                else:
                    bin_nr = np.where(self.lower_edges <= x)[0][-1]
                    self._values[bin_nr] += w
        ...
        def sum(self, x1, x2):
            """
            Perform a binwise integration between the given values
            """
            if x1 >= x2:
...
class BPASS_hist(histogram):
    ...
    def __mul__(self, other):
        """
        Multiply the values with the given number
        """
    ...
    def __div__(self, other):
        """
        Divide the value  with the given number
        """
        self._values = self._values / other
        return None
    ...
    def Fill(self, x, w=1):
        """
        Add data to the BPASS plot. The data has to be in log format.
        """
        x = 10**x /1e9
